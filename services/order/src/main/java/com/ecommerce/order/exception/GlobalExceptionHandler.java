 ...
 * never swallowed into a 500 by the catch-all. Per-type customization of a framework exception is
 * done ONLY via {@code @Override} of the protected hooks; adding a second {@code @ExceptionHandler}
 * for a type the base already maps would crash startup with an ambiguous-mapping error.
 */
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  /**
   * Missing required header. A more-specific subtype of the base's {@code
   * ServletRequestBindingException}, so it wins over the base mapping with no collision — this
   * preserves the {@code IDEMPOTENCY_KEY_REQUIRED} signal that order placement relies on.
   */
  ...
  /**
   * Path-variable / query-param type mismatch → 400 MALFORMED_REQUEST. A standalone handler for
   * {@link MethodArgumentTypeMismatchException} (a distinct subtype of the base's {@code
   * TypeMismatchException}) does NOT collide; unreadable JSON bodies are handled separately via the
   * overridden {@link #handleHttpMessageNotReadable} hook.
   */
