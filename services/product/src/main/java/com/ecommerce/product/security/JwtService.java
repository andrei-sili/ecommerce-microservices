 ...
 * Product never issues tokens — it only validates.
 *
...
public class JwtService {
  ...
  public AuthenticatedUser parse(String token) {
    ...
    return new AuthenticatedUser(subject, roleSet);
  }
  ...
  private static SecretKey loadHmacKey(String secret, boolean hs256Enabled) {
    // Product does not sign, so the secret is required only while HS256 is a validated algorithm.
    // Once the allowlist contracts to RS256 (phase 3), JWT_SECRET may be absent entirely.
    if (!hs256Enabled) {
      return null;
    }
  ...
  private final class AlgKeyLocator extends LocatorAdapter<Key> {
    ...
    protected Key locate(JwsHeader header) {
      ...
      if ("RS256".equals(alg) && rs256Enabled) {
        ...
        // and get(null) throws NPE (unlike HashMap). That NPE would escape parseSignedClaims and
        // surface as a 500 — breaking the pinned 401 contract and handing an unauthenticated
        // caller a scriptable 5xx primitive.
