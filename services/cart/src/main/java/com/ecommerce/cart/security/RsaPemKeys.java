...
/**
 * Loads RSA public keys from X.509/SPKI PEM files with startup fail-fast. Cart only VALIDATES
 * tokens, so it never loads a private key. Every failure throws {@link IllegalStateException} whose
 * message names the config path and the expected PEM format — it <strong>never</strong> echoes key
 * material (base64 body / DER bytes). A bad key therefore fails the bean, which fails the context,
 * which stops the container from becoming ready at deploy time.
 */
final class RsaPemKeys {
  ...
  private static String read(String path) {
    ...
    try {
      ...
      throw new IllegalStateException("JWT public key file could not be read at " + path, e);
    }
  }

  private static byte[] decode(String pem, String path) {
    String base64 =
    ...
    try {
      ...
      throw new IllegalStateException(
          "JWT public key at " + path + " is not a well-formed X.509/SPKI PEM", e);
    }
  }

  private static void requireStrongModulus(BigInteger modulus, String path) {
