...
/**
 * F3(a)'s in-suite guardian: {@code management.endpoint.health.group.readiness.include:
 * readinessState,db} in the SHIPPED yml must keep {@code db} in the readiness group.
 *
 * <p>Why this class exists at all. Before it, deleting {@code db} from that line left this
 * service's ENTIRE suite green — the line was read but unasserted, so nothing in the module could
 * observe readiness ceasing to follow the database. Drop {@code db} again and this class is the
 * thing that goes red. On payment the consequence is direct: readiness is what Kong's upstream
 * health check reads, so a payment pod with a dead database would keep receiving charge requests.
 *
 ...
 * the socketTimeout setting is in place, so reading one as evidence of the other would be a vacuous
 ...
 * by every other integration class. It sets no {@code management.*} property, so the group being
 * asserted can only have come from the shipped file.
 */
@SpringBootTest
...
class ReadinessDbDownIntegrationTest {
   ...
   * point), the placeholder must resolve or no context here can start. The HS256 secret is needed
   * for the same reason in reverse: the test overlay widens {@code accepted-algs} to {@code
   * HS256,RS256}, and {@code JwtService} refuses to construct without a 32-byte secret while HS256
   * is in the allowlist.
   */
  @DynamicPropertySource
    ...
    assertThat(readiness)
        .as(
            "readiness must follow the database down; if this is still 200, 'db' has left the"
                + " readiness group and Kong would keep routing charge requests to a DB-dead pod")
        .isEqualTo(503);
