 ...
 * framework type already declared on the base class would crash startup with an ambiguous-mapping
 * error. The domain handlers below stay direct {@code @ExceptionHandler}s on non-framework or
 * more-specific subtype exceptions, so the money/idempotency/webhook envelopes are untouched.
 */
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  /** Domain errors carry their own status, code and (human) message (402/502/409/422/404 …). */
  ...
  /**
   * Missing required header. More-specific subtype of {@code ServletRequestBindingException} (which
   * the base class handles), so no ambiguous mapping. Preserves the {@code Idempotency-Key}
   * required signal that guards against double-charges.
   */
  ...
  /**
   * Path-var / query-param type mismatch → 400 MALFORMED_REQUEST. Standalone because it is a
   * more-specific subtype of the base class's {@code TypeMismatchException} mapping (no collision),
   * and keeps the original code/message distinct from a generic {@code BAD_REQUEST}.
   */
