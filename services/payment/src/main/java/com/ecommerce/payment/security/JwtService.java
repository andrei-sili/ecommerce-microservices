...
/**
 * Validates inbound access tokens issued by User Service with dual-accept (Slice 5e phase 1).
 * Payment never issues tokens.
 *
...
public class JwtService {
  ...
  public JwtService(JwtProperties properties, MeterRegistry meterRegistry) {
    ...
    // The legacy HMAC secret is required only while HS256 is in the allowlist (payment never signs,
    // so there is no signer-side requirement). When HS256 is dropped, hmacKey is null and the
    // locator never returns it.
    this.hmacKey = hs256Enabled ? loadHmacKey(properties.secret()) : null;
   ...
   * failure. Records the accepted alg/kid on the counter and the {@code jwt.audit} log only after a
   * token FULLY validates.
   */
  ...
  private List<String> extractRoles(Claims claims) {
    ...
    if (raw instanceof List<?> list) {
      // A signed-but-malformed roles claim (e.g. "roles":[null]) is a broken token, not a role
      // named "null": reject it as a MalformedJwtException (→ 401, fail-closed) instead of coercing
      // the null into the string "null" and admitting a token with a bogus authority.
  ...
  private static SecretKey loadHmacKey(String secret) {
    byte[] secretBytes = secret == null ? new byte[0] : secret.getBytes(StandardCharsets.UTF_8);
    if (secretBytes.length < MIN_SECRET_BYTES) {
      throw new IllegalStateException(
          "JWT_SECRET must be at least "
              + MIN_SECRET_BYTES
              + " bytes for HS256 (required because HS256 is in JWT_ACCEPTED_ALGS); configure a"
              + " strong secret via env");
    }
