...
public class JwtService {
  ...
  public JwtService(JwtProperties properties, MeterRegistry meterRegistry) {
    ...
    // so there is no signer-side requirement). When HS256 is dropped, loadHmacKey returns null and
    // the locator never returns it.
  ...
  private static SecretKey loadHmacKey(String secret, boolean hs256Enabled) {
    // HS256 not in the allowlist: payment never signs, so no legacy secret is required — no key.
    if (!hs256Enabled) {
      return null;
    }
